...
## 計画ファイル（`_workingspace/plans/`）

新規起票・スナップショット保存時、ファイル名は **`YYYYMMDD-<slug>.md`** とする。

| 要素 | 規則 |
|------|------|
| **YYYYMMDD** | 起票日（JST）。8 桁・ハイフンなし |
| **slug** | 英小文字＋ハイフン（例: `work-plan`, `lineart-work-plan`） |
| **衝突** | 同日・同 slug → `-v0`, `-v1` 等（例: 初版 `20260614-work-plan-v0.md`、改訂正本 `20260614-work-plan.md`） |

- 計画間リンクは **リネーム後のファイル名** を使う（相対リンク可）。
- 入口一覧・運用例: `_workingspace/plans/README.md`
- 本文は日本語可（開発者向け・Git 管理外）。配置判断は [content-placement スキル](../skills/content-placement/SKILL.md)。
- **公開 docs からのリンク禁止**: `docs/`・ルート README・`samples/` / `assets/` の README 等（Git 追跡ドキュメント）から本ディレクトリへ Markdown リンクを張らない。詳細は [docs-writing.md](../rules/docs-writing.md) §計画書へのリンク禁止。
...
| LineArt JSON の作成・検証・SVG/PNG レンダリング | [ga-lineart](../skills/ga-lineart/SKILL.md) | `graphassist lineart validate` / `graphassist lineart render` / Batch `lineart.render` |
...
- 計画・overview: `_workingspace/plans/20260614-overview.md`, `20260614-work-plan.md`
