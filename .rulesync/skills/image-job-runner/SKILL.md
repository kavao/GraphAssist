...
3. `version`, `input`（または `input_asset`）, `output`, `operations` を持つ JSON を書く。
   ...
   - 出力: `generated/` 配下
   - **カタログ素材** → [catalog-assets スキル](../catalog-assets/SKILL.md)
4. JSON をファイルに保存する（例: `samples/jobs/task.json`）。
   - **1 枚への編集のみ** → 下記 ImageJob 形式
   - **mosaic と job の混在など複数ステップ** → [batch-runner スキル](../batch-runner/SKILL.md) の `commands` 形式
5. dry-run で確認する。
...
→ [catalog-assets スキル](../catalog-assets/SKILL.md)（正本）
...
複数ステップは [batch-runner スキル](../batch-runner/SKILL.md) を使う。
