...
ImageJob `text` は `assets/fonts/` 以下のパスを指定します。

## Git 同梱（最小セット）

clone 直後から次の 2 ファイルが **リポジトリに同梱**されています（`setup-runtime` なしでも birds デモ等が動作）。

| ファイル | ID | 用途 |
|----------|-----|------|
| `DejaVuSans.ttf` | `dejavu-sans` | Latin 等（基本） |
| `PixelMplus12-Regular.ttf` | `pixelmplus12` | ピクセル風日本語（birds デモ等） |

その他のフォント（Noto / 美咲 / Inter 等）は **`setup-runtime` が `runtime/assets/fonts/` に取得**し、`assets/fonts/` にミラーします。
...
| `pixelmplus12` | `PixelMplus12-Regular.ttf` | ピクセル風日本語 | **Git 同梱** / DL | M+ FONT LICENSE |
| `dejavu-sans` | `DejaVuSans.ttf` | Latin 等（基本） | **Git 同梱** / DL | DejaVu / Bitstream Vera License |
...
フォントバイナリ（`.ttf` / `.otf` / `.ttc`）のうち **`DejaVuSans.ttf` と `PixelMplus12-Regular.ttf` のみ Git 管理**します。その他と `*-LICENSE` は `.gitignore` 対象です。`NOTICES.md` は setup 時に再生成されます。
